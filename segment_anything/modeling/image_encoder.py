...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            block = Block(
                ...
                adapter=adapter_train if (i + 1) % 3 == 0 else False,
            )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        x1 = self.neck1(embed_list[0].permute(0, 3, 1, 2))
        x2 = self.neck2(embed_list[1].permute(0, 3, 1, 2))
        x3 = self.neck3(embed_list[2].permute(0, 3, 1, 2))
        x4 = self.neck(x.permute(0, 3, 1, 2))
